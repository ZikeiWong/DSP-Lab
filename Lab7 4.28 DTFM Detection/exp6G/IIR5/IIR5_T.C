...
for(j=0;j<l;j++){
  ...
  for(i=0;i<NX;i++){
  temp1=r1[i];temp2=r2[i];r[i]=temp1+temp2;}
  
  temp1=0;temp2=0;out=0;
  for(k=0;k<4;k++){
    for (i=0; i<2*NBIQ; i++) dbuffer[i] = 0; // clear delay buffer (a must)
    iircas5(r,g[k],out1,&dp,NBIQ, NX);
    ltemp1=out1[NX-1];ltemp2=out1[NX-2];
    ltemp1=(ltemp1*ltemp1+ltemp2*ltemp2+ltemp1*ltemp2*g[k][0]/32767);
    temp1=ltemp1>>15;
      if(temp1>out){out=temp1;row=k;}
  }//for k
  temp1=0;temp2=0;out=0;
  for(k=4;k<8;k++){
    for (i=0; i<2*NBIQ; i++) dbuffer[i] = 0; // clear delay buffer (a must)
    iircas5(r,g[k],out2,&dp,NBIQ, NX);
    ltemp1=out2[NX-1];ltemp2=out2[NX-2];
    ltemp1=(ltemp1*ltemp1+ltemp2*ltemp2+ltemp1*ltemp2*g[k][0]/32767);
    temp2=ltemp1>>15;
